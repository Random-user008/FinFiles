...
def recommend_repos():
    ...
    top_n_indices = get_top_n_indices(topic_distributions,cluster_labels, top_n=20)
    ...
    for index in top_n_indices:
        if(index<len(top_repos_data)):
            if cluster_labels[index] == -1:
                continue
            print(f"{index}:{top_repos_data.iloc[index]['name']} ({top_repos_data.iloc[index]['description']}) - {top_repos_data.iloc[index]['url']}")
